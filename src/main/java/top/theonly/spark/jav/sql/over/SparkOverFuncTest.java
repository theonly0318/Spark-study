 ...
 * @author theonly
 */
public class SparkOverFuncTest {
...
//          .config("hive.metastore.uris", "thrift://node1:9083")
        ...
        // 创建表tb_sales
        session.sql("create table if not exists tb_sales (riqi string, leibie string, jine Int)" + "row format delimited fields terminated by '\t'");
        // 加载数据到hive表中
        session.sql("load data local inpath '/root/spark/test/sales.txt' into table tb_sales");
        // 使用开窗函数 over() 对类别分组，金额倒序排序
        Dataset<Row> dataset = session.sql("select riqi, leibie, jine, row_number() over(partition by leibie order by jine desc) as rank from tb_sales");
        // 注册临时表
        dataset.registerTempTable("tmp");
        // 查询每个类别每日金额前三的数据
        Dataset<Row> result = session.sql("select riqi, leibie, jine from tmp where rank <= 3");
        // 将结果写到result表中
