 ...
 *  线性不可分 ----升高维度
 */
object LogisticRegressionTest3 {

  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("LogisticRegression3").setMaster("local[3]")
    val sc = new SparkContext(conf)
    // 解决线性不可分我们来升维,升维有代价,计算复杂度变大了
    val inputData :RDD[LabeledPoint]= MLUtils.loadLibSVMFile(sc, "./data/MLlib/线性不可分数据集.txt")
      .map { labelpoint =>
        ...
        val convertFeature: linalg.Vector = Vectors.dense(array)
