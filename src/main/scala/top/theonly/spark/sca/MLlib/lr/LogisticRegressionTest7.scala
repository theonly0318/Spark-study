 ...
 * 最大最小值归一化
 */
object LogisticRegressionTest7 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("LogisticRegression7").master("local").getOrCreate()
    spark.sparkContext.setLogLevel("Error")
    /**
     * 加载生成的DataFrame自动有两列：label features
     */
    val df = spark.read.format("libsvm").load("./data/MLlib/环境分类数据.txt")
    /**
