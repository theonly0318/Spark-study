 ...
 * 提高模型抗干扰能力
 */
object LogisticRegressionTest5 {

  def main(args: Array[String]) {

    val conf = new SparkConf().setAppName("LogisticRegression5").setMaster("local[3]")
    val sc = new SparkContext(conf)
    val inputData = MLUtils.loadLibSVMFile(sc, "./data/MLlib/健康状况训练集.txt")
    val splits = inputData.randomSplit(Array(0.8, 0.2),100L)
